...
public class GuiManager {
    ...
    public void openNpcSelectionGUI(Player player) {
        ...
        plugin.getPlayerCurrentPage().put(player.getUniqueId(), currentPage);
        ...
        // Pagination buttons - Fixed logic
        if (currentPage > 0) {
        ...
        if (pageInfoMeta != null) {
            ...
            displayName = displayName.replace("{total_pages}", String.valueOf(Math.max(totalPages, 1)));
    ...
    public void openNpcTradeManagementGUI(Player player, String npcId) {
        ...
        int slotIndex = 0;
        // Sort trade IDs naturally (so "1", "2", "10" instead of "1", "10", "2")
        List<String> sortedTradeIds = new ArrayList<>(npcTradeMap.keySet());
        sortedTradeIds.sort((a, b) -> {
            try {
        ...
        for (String tradeId : sortedTradeIds) {
    ...
    public void openNpcShopGUI(Player player, String npcId) {
        ...
        // Sort trade IDs consistently for shop display
        List<String> sortedTradeIds = new ArrayList<>(npcTradeMap.keySet());
        sortedTradeIds.sort((a, b) -> {
            try {
        ...
        // Fix pagination bounds for shop
        if (currentPage < 0) {
            currentPage = 0;
        } else if (currentPage >= totalPages && totalPages > 0) {
            currentPage = totalPages - 1;
    ...
    private void drawControlPanel(Inventory gui, Player player, int currentPage, int totalPages) {
        // Row 0: Previous page button (slot 7)
        if (currentPage > 0) {
            ItemStack prevButton = createConfiguredItem("npc-shop.control-panel.prev-page", "", "");
            gui.setItem(7, prevButton);
        } else {
            ItemStack placeholder = createConfiguredItem("npc-shop.control-panel.placeholder", "", "");
            gui.setItem(7, placeholder);
        }

        // Row 1: Page info (slot 16)
        ...
        if (pageInfoMeta != null) {
            String name = pageInfoMeta.getDisplayName()
                    ...
                    .replace("{total_pages}", String.valueOf(Math.max(totalPages, 1)));
            pageInfoMeta.setDisplayName(plugin.getConfigManager().translateColors(name));
            pageInfo.setItemMeta(pageInfoMeta);
        }
        gui.setItem(16, pageInfo);

        // Row 2: Next page button (slot 25)
        if (currentPage < totalPages - 1) {
            ItemStack nextButton = createConfiguredItem("npc-shop.control-panel.next-page", "", "");
            gui.setItem(25, nextButton);
        } else {
            ItemStack placeholder = createConfiguredItem("npc-shop.control-panel.placeholder", "", "");
            gui.setItem(25, placeholder);
        }

        // Row 3: Player info (slot 34)
        ...
        gui.setItem(34, playerInfo);

        // Row 4: Help button (slot 43)
        ItemStack helpButton = createConfiguredItem("npc-shop.control-panel.help", "", "");
        gui.setItem(43, helpButton);

        // Row 5: Close button (slot 52)
        ItemStack closeButton = createConfiguredItem("npc-shop.control-panel.close", "", "");
        gui.setItem(52, closeButton);
    }
